    ...
    //Current volume is not currently set to match temperature value, This will need to be further implemented depending on how we want it to work
{
    ...
    public void Start() //Play sounds on start (main songs and ambience)
    {
        FindObjectOfType<Audio_Manager>().Spawn(this.gameObject ,"Cave");
        FindObjectOfType<Audio_Manager>().Spawn(this.gameObject, "Main");
        FindObjectOfType<Audio_Manager>().Spawn(this.gameObject, "Ice");
        FindObjectOfType<Audio_Manager>().Spawn(this.gameObject, "Heat");
        FindObjectOfType<Audio_Manager>().Play(this.gameObject);
    }
    void Update()
    {
        ...
        {
            FindObjectOfType<Audio_Manager>().Play("Lazer");
        }
        ...
        FindObjectOfType<Audio_Manager>().SetVolume("Main", main_volume);
        FindObjectOfType<Audio_Manager>().SetVolume("Ice", ice_volume);
        FindObjectOfType<Audio_Manager>().SetVolume("Heat", heat_volume);
    }
